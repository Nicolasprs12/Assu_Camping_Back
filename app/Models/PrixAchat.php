...
class PrixAchat extends Model {
    ...
    protected $table = 'prix_achat';
    protected $id = 'idPrixAchat';

	private $idPrixAchat ;				// nombre
    ...
    public function setAll($result = array())
    {
        ...
        if (count($result) > 0) {
            $this->idPrixAchat = $result['idPrixAchat'];
    ...
    public function lire()
    {
        ...
        $result = $this->query($sql, [':id' => $this->idPrixAchat], true); // Récupère un seul enregistrement correspondant
        $num = count($result);
        var_dump($num);

        if ($num == 1) {
            $this->idPrixAchat = $result[0][$this->id];
