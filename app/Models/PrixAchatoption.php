...
class PrixAchatOption extends Model {

    protected $table = 'prix_achat_option';

    protected $id = 'idPrixAchatOption';

	private $idPrixAchatOption ;		// nombre
    ...
    public function setAll($result = array())
    {
        ...
        if (count($result) > 0) {
            $this->idPrixAchatOption = $result['idPrixAchatOption'];
    ...
    public function lire()
    {
        ...
        $result = $this->query($sql, [':id' => $this->idPrixAchatOption], true); // Récupère un seul enregistrement correspondant
        $num = count($result);
        var_dump($num);

        if ($num == 1) {
            $this->idPrixAchatOption = $result[0][$this->id];
