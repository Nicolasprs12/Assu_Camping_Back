...
class PrixVenteOption extends Model {

    protected $table = 'prix_vente_option';
    protected $id = 'idPrixVenteOption';

	private $idPrixVenteOption ;		// nombre
    private $idFournisseur;				// nombre
	private $assistance ;				// bool
	private $dateCreation;				// objet date, heure
    private $typeContrat;				// chaîne de caractères
    private $duree;						// nombre
    private $prix;						// décimal
    ...
    public function setAll($result = array())
    {
        ...
        if (count($result) > 0) {
            $this->idPrixVenteOption = $result['idPrixVenteOption'];
    ...
    public function lire()
    {
        ...
        $result = $this->query($sql, [':id' => $this->idPrixVenteOption], true); // Récupère un seul enregistrement correspondant
        $num = count($result);
        var_dump($num);

        if ($num == 1) {
            $this->idPrixVenteOption = $result[0][$this->id];
